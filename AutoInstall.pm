...
the user, if the user chooses not to install the modules that belong to
it.  This differs from the pre-0.26 'silent install' behaviour.
...
The user-defined C<MY::postamble>, if present, is responsible to call
C<ExtUtils::AutoInstall::postamble> and include the output in its
return value.
